...
const NewUser = dynamic(() => import("../AddCard/NewUser"));
...
export default function Users({ users }) {
  ...
  const fetchUsers = async () => {
    if (!searchQuery) {
      ...
      const dataFiltered = users.filter((user) => user.name.toLowerCase().includes(searchQuery.toLowerCase()) || user.email.toLowerCase().includes(searchQuery.toLowerCase()) || user.role.toLowerCase().includes(searchQuery.toLowerCase()));
      setAllUsers(dataFiltered);
    }
  }
  ...
  const handleKeyDown = async(e) => {
    if (e.keyCode === 13) {
      if (!searchQuery) {
        ...
        const dataFiltered = users.filter((user) => user.name.toLowerCase().includes(searchQuery.toLowerCase()) || user.reference.toLowerCase().includes(searchQuery.toLowerCase()) || user.country.toLowerCase().includes(searchQuery.toLowerCase()) || user.email1.toLowerCase().includes(searchQuery.toLowerCase()) || user.phone1.toLowerCase().includes(searchQuery.toLowerCase()));
        setAllUsers(dataFiltered);
      }
    }
  }
